...
| dock | `op` | `621339cf` | `plugin/src/Caelestia/Config/dockconfig.hpp` | same | `reimplemented` | Ported from OP's `ConfigObject` to `settings::ObjectNode`, the system upstream rewrote in Phase 2 (D7); keys and defaults unchanged. Needed three registrations OP's layout did not: the module source list, `rootnodes.hpp`, and the *attached* `Config.dock` accessor -- the last is why `Config.dock.enabled` resolved as a type but not as a member. |
| dock | `op` | `621339cf` | `modules/nexus/pages/panels/DockPanel.qml` | same | `adapted` | `Icons.getIcon` does not exist here **or in OP** -- replaced with `Quickshell.iconPath`, this tree's idiom. `ScriptModel.values` wants `QList<QJSValue>` and `pinnedApps` is a `QStringList`, so it is spread. Registered in the Panels `StackPage`. |
| dock | _(ours)_ | — | `modules/bar/Bar.qml`, `modules/drawers/Regions.qml`, `components/ScreenState.qml` | same | `adapted` | MiDnight's side is a bar entry, now filtered out unless the variant selects it -- otherwise the machine shows two docks. A region is added for OP's panel, and `ScreenState` gains the `dock` visibility flag its wrapper reads, which also makes `qs ipc call drawers toggle dock` work. |
