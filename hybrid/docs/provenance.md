...
| hotspot | `op` | `621339cf` | `services/Hotspot.qml` | `services/Hotspot.qml` | `adapted` | Two changes. `pollTimer.running` was an unconditional `true`, which runs `nmcli` every 10s forever once the singleton exists, active or not; it is now gated on `GlobalConfig.hybrid.features.hotspot` (adding `import Caelestia.Config`). The rest is mechanical: `qml-section-order.py` moved functions and properties above the child objects, and `qmlformat` rejoined wrapped ternaries. |
