...
| lock power confirm | `op` | `621339cf` | `modules/lock/PowerConfirm.qml` | same | `adapted` | OP's `accept()` hardcoded `Config.session.commands.shutdown`, because its lock screen has one dedicated power button. Ours renders `Config.session.buttons`, a list with a command each, so the dialog now takes the command it is confirming and covers all of them. Gated on `hybrid.features.lockPowerConfirm`; without it the lock screen runs session commands immediately, as it did before. |
