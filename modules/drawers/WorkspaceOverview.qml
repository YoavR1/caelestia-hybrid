...
Item {
    ...
    // The panel itself always exists: modules/drawers aliases it (Panels.qml), reads its
    // offsetScale for a region (Regions.qml) and feeds a deform matrix from it
    // (ContentWindow.qml). Making the root conditional would leave all three on a null,
    // which is the alias graph T1 exists to warn about.
    //
    // Its *content* is another matter, and that is where the cost is -- a Repeater tracking
    // every toplevel. With OP's overview selected this panel is never opened, so the content
    // has no reason to be built.
    Loader {
        anchors.fill: parent
        active: GlobalConfig.hybrid.variants.overview === HybridVariant.Midnight
                    ...
                    VerticalFadeListView {
                        ...
                        delegate: Item {
                            ...
                            property var hlMonitor: {
                                ...
                                if (ws && ws.monitor)
                                    return ws.monitor.lastIpcObject;
                            ...
                            property real inactiveOffsetX: {
                                if (isActive || windows.length === 0)
                                    return 0;
                                let firstWindow = windows[0].lastIpcObject;
                                if (!firstWindow || !firstWindow.at || !firstWindow.size)
                                    return 0;
